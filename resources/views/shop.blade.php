<x-layout>
    ...
    <main class="pt-32 pb-20 max-w-7xl mx-auto px-6">
        <div class="mb-16">
            ...
            <p class="text-zinc-500 dark:text-zinc-400 font-medium max-w-xl">Explore our latest drops and architectural denim pieces.</p>
        </div>
        ...
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                @foreach($products as $product)
                    <div class="group">
                        <div class="relative aspect-[4/5] overflow-hidden rounded-2xl bg-zinc-100 dark:bg-zinc-900 mb-6 transition-transform duration-500 hover:-translate-y-2">
                            @if($product->image)
                            ...
                            <div class="absolute top-4 right-4">
                                <span class="px-3 py-1 bg-white/90 dark:bg-black/90 backdrop-blur rounded-full text-xs font-bold uppercase tracking-widest shadow-xl">{{ $product->category ?? 'General' }}</span>
                            </div>
                        </div>
                        ...
                        <p class="text-zinc-500 dark:text-zinc-400 text-sm font-medium line-clamp-2 mb-6">{{ $product->description }}</p>
                        <button class="w-full py-4 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 font-black rounded-lg transition-all hover:bg-red-600 hover:text-white active:scale-95 uppercase">
                            Add to Cart
                        </button>
                    </div>
                @endforeach
